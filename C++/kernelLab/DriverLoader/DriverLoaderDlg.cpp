...
BEGIN_MESSAGE_MAP(CAboutDlg, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON_INSTALL, &CAboutDlg::OnBnClickedButtonInstall)
END_MESSAGE_MAP()
...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT_FILE, my_syspath);
}
...
BOOL CDriverLoaderDlg::OnInitDialog()
{
	...
	my_syspath.EnableFileBrowseButton(_T(""), _T("SYS Files(*.sys)| *.sys|All Files (*.*)|*.*||"));
...
void CDriverLoaderDlg::OnEnChangeEditFile()
{
...
void CAboutDlg::OnBnClickedButtonInstall()
{
	// TODO: 在此添加控件通知处理程序代码
}
...
void CDriverLoaderDlg::OnBnClickedButtonstart()
{
	// TODO: 在此添加控件通知处理程序代码
		// TODO: 在此添加控件通知处理程序代码
	CString fileAddress;
	GetDlgItemText(IDC_EDIT_FILE, fileAddress);
	if (!fileAddress.IsEmpty()) {
		if (!SystemServiceOperate(fileAddress, 1)) {
			chMB("启动服务失败！");
		}
	}
	else {
...
void CDriverLoaderDlg::OnBnClickedButtonStop()
{
	...
	GetDlgItemText(IDC_EDIT_FILE, fileAddress);
	if (!fileAddress.IsEmpty()) {
		if (!SystemServiceOperate(fileAddress, 2)) {
			chMB("停止服务失败！");
		}
	}
	else {
...
void CDriverLoaderDlg::OnBnClickedButtonUnload()
{
	...
	GetDlgItemText(IDC_EDIT_FILE, fileAddress);
	if (!fileAddress.IsEmpty()) {
		if (!SystemServiceOperate(fileAddress, 3)) {
			chMB("卸载服务失败！");
		}
	}
	else {
...
void CDriverLoaderDlg::OnBnClickedButtonInstall()
{
	...
	GetDlgItemText(IDC_EDIT_FILE, fileAddress);
	if (!fileAddress.IsEmpty()) {
		if (!SystemServiceOperate(fileAddress, 0)) {
			chMB("创建服务失败！");
		}
	}
	else {
